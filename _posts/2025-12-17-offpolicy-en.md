...
That is the central off-policy question: **when can data collected by older policies still support reliable improvement of a newer one?**

This post studies exactly that question: under what conditions can a mixture of data from multiple policy versions still support a monotonic-improvement guarantee?
...
This is an average-divergence / average-TV style bound. It is closer in spirit to CPO / Achiam et al. (2017) than to the classic TRPO presentation based on $\max_s D_{\mathrm{TV}}$. I use it here because it lands more naturally on sample averages and is easier to carry into the multi-source setting.
...
> **Observation 5.2 (Infeasibility of Update Shift Constraints)**
>
...
Standard PPO constrains $D_{\mathrm{TV}}(\pi_{k+1}, \pi^{(i)})$, requiring the new policy to be close to every sampling-source policy at once. By Observation 5.2, when the old policies $\pi^{(1)}, \pi^{(2)}, \ldots$ differ substantially, **no $\pi_{k+1}$ can satisfy all of those constraints simultaneously**. The result is a trust-region intersection that shrinks, or even becomes empty, with the most stale policy becoming the bottleneck.
...
2. **Two constraint methods**: Method 1 (adaptive clipping center) and Method 2 (fixed incremental clipping) correspond to two different ways of controlling $U_k$
3. **Comparison with standard PPO**: Standard PPO constrains $D_{\mathrm{TV}}(\pi_{k+1}, \pi^{(i)})$, which becomes structurally problematic under multi-policy mixture; Methods 1/2 instead target $D_{\mathrm{TV}}(\pi_{k+1}, \pi_k)$
