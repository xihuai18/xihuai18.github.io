...
> **定理1.1（策略性能改进下界）**
> 
> 定义期望优势上界常数 $C_{\pi,\pi_k} := \max_{s} | \mathbb{E}_{a \sim \pi}[A^{\pi_k}(s,a)] |$，则：
> 
> $$
...
这意味着：**如果我们能让每个样本上 $|\rho_{k+1} - \rho_k| \leq \epsilon$，就能保证 $U_k \leq \epsilon/2$**。
...
如果约束 $\left|\frac{\pi_{k+1}(a|s)}{\pi_k(a|s)} - 1\right| \leq \epsilon$，由于 $\mathbb{E}_{a\sim\pi^{(i)}}[\rho_k] = 1$，可证 $U_k \leq \epsilon/2$。

这种方法直接对 $\pi_{k+1}/\pi_k$ 以 1 为中心裁剪，**完全不涉及旧策略 $\pi^{(i)}$**。

### 5.4 三种裁剪机制的目标函数

为了对比，我们给出三种裁剪机制的完整目标函数。设当前样本来自旧策略 $\pi^{(i)}$，记：
...
- 方法二的被积函数为 $\rho_k \cdot |r - 1|$，即便 $|r-1| \leq \epsilon$，被积函数仍可达 $\epsilon \cdot \rho_k$，产生尖峰。
- 方法一直接约束 $|\rho_{k+1} - \rho_k| \leq \epsilon$，被积函数上界恒为 $\epsilon$，不受 $\rho_k$ 放大。
...
- 方法一约束 $|\pi_{k+1} - \pi_k| \leq \epsilon \cdot \pi^{(i)}$，这是**加法型约束**：若该token在旧策略下概率较高（如 $\pi^{(i)}(a|s) = 0.1$），即便当前概率很低，也允许较快提升。
...
设定阈值 $\epsilon_{\mathrm{stale}}$，对每个样本计算 $|\rho_k - 1| = |\pi_k(a|s)/\pi^{(i)}(a|s) - 1|$，丢弃超过阈值者。
...
由引理3.3，$\hat{S}_k$ 可表示为样本级可计算形式。给定阈值 $\epsilon_{\mathrm{stale}}$，若训练仅使用满足 $|\pi_k(a|s)/\hat{\pi}^{(i)}(a|s) - 1| \leq \epsilon_{\mathrm{stale}}$ 的样本，则 $\hat{S}_k \leq \epsilon_{\mathrm{stale}}/2$。
...
- **裁剪半径 $\epsilon$**：控制 $U_k$，典型值 0.1–0.2
- **陈旧性阈值 $\epsilon_{\mathrm{stale}}$**：控制 $S_k$，典型值 0.3–0.5；强异步系统可放宽至 0.8–1.0
- **版本窗口 $W$**：限制混合采样的策略版本数量，典型值 2–4
