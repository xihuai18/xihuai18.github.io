...
function initAboutPageDividers() {
  // Check if we're on About page (has .post but also has .publications inside it)
  ...
  const allPubs = bibliography.querySelectorAll('li');
  if (allPubs.length === 1) {
    allPubs[0].classList.add('single-pub-in-year');
  }
}
